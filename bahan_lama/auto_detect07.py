...
def detect_and_extract_text_bubbles(image_path):
    ...
    output_folder = 'extracted_bubbles01'
                ...
                if ocr_result:  # Hanya simpan jika ada teks
                    ...
                    for i in range(y, y + h):
                        for j in range(x, x + w):
                            ...
                            # Jika piksel mendekati putih
                            if np.all(pixel >= [200, 200, 200]):  
                                transparent_canvas[i, j] = [0, 0, 0, 0]  # Transparan

                            # Jika piksel mendekati hitam
                            elif np.all(pixel <= [50, 50, 50]):  
                                transparent_canvas[i, j, :3] = [255, 0, 0]  # Ubah jadi merah
                                transparent_canvas[i, j, 3] = 255  # Opasitas penuh

                            # Warna lainnya
                            else:
                                transparent_canvas[i, j, :3] = pixel  # Warna asli
                                transparent_canvas[i, j, 3] = 255  # Opasitas penuh
    ...
    return {
        'detection_result': 'merged_bubbles.jpg',
